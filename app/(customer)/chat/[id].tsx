...
export default function CustomerChatDetailScreen() {
  ...
  const [otherUserName, setOtherUserName] = useState("Technician");
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select(`
        technician_id,
        profiles!bookings_technician_id_fkey (full_name)
      `)
      .eq("id", bookingId)
      .single()
      .then(({ data }) => {
        if (data) {
          setOtherUserId((data as any).technician_id);
          setOtherUserName((data as any).profiles?.full_name ?? "Technician");
        }
      });
  }, [bookingId]);
  ...
  const sendMessage = async () => {
    ...
    if (!text || !user || !bookingId || !otherUserId) return;
    setSending(true);
    setInputText("");
    try {
  ...
  const messagesWithSeparators: Array<{ type: "message"; data: Message } | { type: "separator"; label: string; key: string }> = [];
  let lastDateLabel = "";
          ...
          >
            {messages.length === 0 && (
              <View style={{ alignItems: "center", paddingTop: 48 }}>
                ...
                <Text style={{ color: "#C4C9D4", fontSize: 13, marginTop: 4 }}>Say hello to start the conversation!</Text>
              </View>
            )}
